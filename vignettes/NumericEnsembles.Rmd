...
Here is a link to an example where 49 college teams submitted 699 entries on the Boston Housing data set. The public results may be seen here: <https://www.kaggle.com/competitions/uou-g03784-2022-spring/leaderboard?tab=public>
...
• Bagged Random Forest (called bag_rf in the package)
...
• Boosed Random Forest (called boost_rf in the package)
...
• Generalized Additive Models (called GAM in the package)

• Gradient Boosted (called gb in the package)

• K-Nearest Neighbors (tuned), (called knn in the package)
...
• Partial Least Squares (called pls in the package)

• Principal Components Regression (called pcr in the package)

• Random Forest (called rf in the package)

• Ridge

• RPart

• Support Vector Machines (called svm in the package)
...
• Ensemble Bagged Random Forest (called ensemble_bag_rf in the package)

• Ensemble Bagging

• Ensemble Bayes GLM (called ensemble_bayesglm in the package)

• Ensemble BayesRNN (called ensemble_bayesrnn in the package)

• Ensemble Boosted Random Forest (called ensemble_boost_rf in the package)
...
• Ensemble K-Nearest Neighbors (tuned)
...
Summary: NumericEnsembles accomplishes all of these tasks in one code chunk, and typically the best model here beats the best models from previously published results. For [[example:\\\\](example:){.uri}](%5Bexample:%5D(example:)%7B.uri%7D){.uri}\
...
Calculation of the percentage decrease (from <https://www.calculatorsoup.com/calculators/algebra/percent-change-calculator.php>)\
Calculate percentage change \
from V~1~ = 2.09684 to V~2~ = 0.1135 
