...
#' @importFrom stats as.formula complete.cases cor lm sd predict residuals reorder quantile gaussian
...
Numeric <- function(data, colnum, numresamples, how_to_handle_strings = c(0("none"), 1("factor levels")), do_you_have_new_data = c("Y", "N"),
  ...
  cppls_train_RMSE <- 0
  cppls_test_RMSE <- 0
  cppls_validation_RMSE <- 0
  cppls_sd <- 0
  cppls_overfitting <- 0
  cppls_duration <- 0
  cppls_duration_mean <- 0
  cppls_holdout_mean <- 0
  cppls_holdout_RMSE <- 0
  cppls_holdout_RMSE_mean <- 0
  cppls_bias <- 0
  cppls_MAE <- 0
  cppls_MSE <- 0
  cppls_SSE <- 0
  ...
  for (i in 1:numresamples) {
    ...
    ####  Model #6 Canonical Powered Partial Least Squares (CPPLS) ####
    cppls_start <- Sys.time()
    cppls_train_fit <- pls::cppls(y ~ ., data = train)
    cppls_train_RMSE[i] <- Metrics::rmse(actual = train$y, predicted = predict(
      object = cppls_train_fit,
      newdata = train
    ))
    cppls_train_RMSE_mean <- mean(cppls_train_RMSE)
    cppls_test_RMSE[i] <- Metrics::rmse(actual = test$y, predicted = predict(
      object = cppls_train_fit,
      newdata = test
    ))
    cppls_test_RMSE_mean <- mean(cppls_test_RMSE)
    cppls_validation_RMSE[i] <- Metrics::rmse(actual = validation$y, predicted = predict(
      object = cppls_train_fit,
      newdata = validation
    ))
    cppls_validation_RMSE_mean <- mean(cppls_validation_RMSE)
    cppls_holdout_RMSE[i] <- mean(cppls_test_RMSE_mean, cppls_validation_RMSE_mean)
    cppls_holdout_RMSE_mean <- mean(cppls_holdout_RMSE)
    cppls_holdout_RMSE_sd_mean <- sd(c(cppls_test_RMSE_mean, cppls_validation_RMSE_mean))
    cppls_train_predict_value <- as.numeric(predict(object = cppls_train_fit, newdata = train))
    cppls_test_predict_value <- as.numeric(predict(object = cppls_train_fit, newdata = test))
    cppls_validation_predict_value <- as.numeric(predict(object = cppls_train_fit, newdata = validation))
    cppls_predict_value_mean <- mean(c(cppls_test_predict_value, cppls_validation_predict_value))
    cppls_sd[i] <- sd(c(cppls_test_predict_value, cppls_validation_predict_value))
    cppls_sd_mean <- mean(cppls_sd)
    cppls_overfitting[i] <- cppls_holdout_RMSE_mean / cppls_train_RMSE_mean
    cppls_overfitting_mean <- mean(cppls_overfitting)
    cppls_overfitting_range <- range(cppls_overfitting)
    cppls_overfitting_sd <- sd(cppls_overfitting)
    y_hat_cppls <- c(cppls_test_predict_value, cppls_validation_predict_value)
    cppls_bias[i] <- Metrics::bias(actual = c(test$y, validation$y), predicted = c(cppls_test_predict_value, cppls_validation_predict_value))
    cppls_bias_mean <- mean(cppls_bias)
    cppls_bias_sd <- sd(cppls_bias)
    cppls_MAE[i] <- Metrics::mae(actual = c(test$y, validation$y), predicted = c(cppls_test_predict_value, cppls_validation_predict_value))
    cppls_MAE_mean <- mean(cppls_MAE)
    cppls_MAE_sd <- sd(cppls_MAE)
    cppls_MSE[i] <- Metrics::mse(actual = c(test$y, validation$y), predicted = c(cppls_test_predict_value, cppls_validation_predict_value))
    cppls_MSE_mean <- mean(cppls_MSE)
    cppls_MSE_sd <- sd(cppls_MSE)
    cppls_SSE[i] <- Metrics::sse(actual = c(test$y, validation$y), predicted = c(cppls_test_predict_value, cppls_validation_predict_value))
    cppls_SSE_mean <- mean(cppls_SSE)
    cppls_SSE_sd <- sd(cppls_SSE)
    cppls_ks_p_value[i] <- stats::ks.test(x = y_hat_cppls, y = c(train$y, validation$y), exact = TRUE)$p.value
    cppls_ks_p_value_mean <- mean(cppls_ks_p_value)
    cppls_ks_p_value_sd <- sd(cppls_ks_p_value)
    cppls_ks_stat[i] <- stats::ks.test(x = y_hat_cppls, y = c(train$y, validation$y), exact = TRUE)$statistic
    cppls_ks_stat_mean <- mean(cppls_ks_stat)
    cppls_ks_test <- c(cppls_ks_stat_mean, cppls_ks_p_value_mean)

    cppls_end <- Sys.time()
    cppls_duration[i] <- cppls_end - cppls_start
    cppls_duration_mean <- mean(cppls_duration)
    cppls_duration_sd <- sd(cppls_duration)
    ...
    ensemble <- data.frame(
      ...
      "CPPLS" = y_hat_cppls * 1 / cppls_holdout_RMSE_mean,
  ...
  summary_results <- data.frame(
    "Model" = c(
      ...
      "BoostRF", "CPPLS", "Cubist", "Earth", "Elastic", "GAM", "Gradient Boosted", "KNN", "Lasso", "Linear", "PLS",
    ...
    "Mean_holdout_RMSE" = round(c(
      ...
      bayesrnn_holdout_RMSE_mean, boost_rf_holdout_RMSE_mean, cppls_holdout_RMSE_mean, cubist_holdout_RMSE_mean, earth_holdout_RMSE_mean, elastic_holdout_RMSE_mean,
      gam_holdout_RMSE_mean, gb_holdout_RMSE_mean, knn_holdout_RMSE_mean, lasso_holdout_RMSE_mean, linear_holdout_RMSE_mean,
      pls_holdout_RMSE_mean, pcr_holdout_RMSE_mean, rf_holdout_RMSE_mean,
    ...
    "Std_Deviation_of_holdout_RMSE" = round(c(
      ...
      bayesrnn_holdout_RMSE_sd_mean, boost_rf_holdout_RMSE_sd_mean, cppls_holdout_RMSE_sd_mean, cubist_holdout_RMSE_sd_mean, earth_holdout_RMSE_sd_mean, elastic_holdout_RMSE_sd_mean,
      ...
      pls_holdout_RMSE_sd_mean, pcr_holdout_RMSE_sd_mean, rf_holdout_RMSE_sd_mean,
    ...
    "KS_Test_Stat_mean" = round(c(
      ...
      bayesrnn_ks_stat_mean, boost_rf_ks_stat_mean, cppls_ks_stat_mean, cubist_ks_stat_mean, earth_ks_stat_mean, elastic_ks_stat_mean,
      gam_ks_stat_mean, gb_ks_stat_mean, knn_ks_stat_mean, lasso_ks_stat_mean, linear_ks_stat_mean,
      pls_ks_stat_mean, pcr_ks_stat_mean, rf_ks_stat_mean,
    ...
    "KS_Test_P_Value_mean" = round(c(
      ...
      bayesrnn_ks_p_value_mean, boost_rf_ks_p_value_mean, cppls_ks_p_value_mean, cubist_ks_p_value_mean, earth_ks_p_value_mean, elastic_ks_p_value_mean,
      gam_ks_p_value_mean, gb_ks_p_value_mean, knn_ks_p_value_mean, lasso_ks_p_value_mean, linear_ks_p_value_mean,
      pls_ks_p_value_mean, pcr_ks_p_value_mean, rf_ks_p_value_mean,
    ...
    "KS_Test_P_Value_std_dev" = round(c(
      ...
      bayesrnn_ks_p_value_sd, boost_rf_ks_p_value_sd, cppls_ks_p_value_sd, cubist_ks_p_value_sd, earth_ks_p_value_sd, elastic_ks_p_value_sd,
      gam_ks_p_value_sd, gb_ks_p_value_sd, knn_ks_p_value_sd, lasso_ks_p_value_sd, linear_ks_p_value_sd,
      pls_ks_p_value_sd, pcr_ks_p_value_sd, rf_ks_p_value_sd,
    ...
    "Bias" = round(c(
      ...
      bayesrnn_bias_mean, boost_rf_bias_mean, cppls_bias_mean, cubist_bias_mean, earth_bias_mean, elastic_bias_mean,
      gam_bias_mean, gb_bias_mean, knn_bias_mean, lasso_bias_mean, linear_bias_mean,
      pls_bias_mean, pcr_bias_mean, rf_bias_mean,
    ...
    "Bias_sd" = round(c(
      ...
      bayesrnn_bias_sd, boost_rf_bias_sd, cppls_bias_sd, cubist_bias_sd, earth_bias_sd, elastic_bias_sd,
      gam_bias_sd, gb_bias_sd, knn_bias_sd, lasso_bias_sd, linear_bias_sd,
      pls_bias_sd, pcr_bias_sd, rf_bias_sd,
    ...
    "Mean_MAE" = round(c(
      ...
      bayesrnn_MAE_mean, boost_rf_MAE_mean, cppls_MAE_mean, cubist_MAE_mean, earth_MAE_mean, elastic_MAE_mean,
      gam_MAE_mean, gb_MAE_mean, knn_MAE_mean, lasso_MAE_mean, linear_MAE_mean,
      pls_MAE_mean, pcr_MAE_mean, rf_MAE_mean,
    ...
    "Mean_MAE_sd" = round(c(
      ...
      bayesrnn_MAE_sd, boost_rf_MAE_sd, cppls_MAE_sd, cubist_MAE_sd, earth_MAE_sd, elastic_MAE_sd,
      gam_MAE_sd, gb_MAE_sd, knn_MAE_sd, lasso_MAE_sd, linear_MAE_sd,
      pls_MAE_sd, pcr_MAE_sd, rf_MAE_sd,
    ...
    "Mean_MSE" = round(c(
      ...
      bayesrnn_MSE_mean, boost_rf_MSE_mean, cppls_MSE_mean, cubist_MSE_mean, earth_MSE_mean, elastic_MSE_mean,
      gam_MSE_mean, gb_MSE_mean, knn_MSE_mean, lasso_MSE_mean, linear_MSE_mean,
      pls_MSE_mean, pcr_MSE_mean, rf_MSE_mean,
    ...
    "Mean_MSE_sd" = round(c(
      ...
      bayesrnn_MSE_sd, boost_rf_MSE_sd, cppls_MSE_sd, cubist_MSE_sd, earth_MSE_sd, elastic_MSE_sd,
      gam_MSE_sd, gb_MSE_sd, knn_MSE_sd, lasso_MSE_sd, linear_MSE_sd,
      pls_MSE_sd, pcr_MSE_sd, rf_MSE_sd,
    ...
    "Mean_SSE" = round(c(
      ...
      bayesrnn_SSE_mean, boost_rf_SSE_mean, cppls_SSE_mean, cubist_SSE_mean, earth_SSE_mean, elastic_SSE_mean,
      gam_SSE_mean, gb_SSE_mean, knn_SSE_mean, lasso_SSE_mean, linear_SSE_mean,
      pls_SSE_mean, pcr_SSE_mean, rf_SSE_mean,
    ...
    "Mean_SSE_sd" = round(c(
      ...
      bayesrnn_SSE_sd, boost_rf_SSE_sd, cppls_SSE_sd, cubist_SSE_sd, earth_SSE_sd, elastic_SSE_sd,
      gam_SSE_sd, gb_SSE_sd, knn_SSE_sd, lasso_SSE_sd, linear_SSE_sd,
      pls_SSE_sd, pcr_SSE_sd, rf_SSE_sd,
    ...
    "Mean_data" = round(c(
      ...
      bayesrnn_predict_value_mean, boost_rf_predict_value_mean, cppls_predict_value_mean, cubist_predict_value_mean, earth_predict_value_mean, elastic_test_predict_value_mean,
    ...
    "Std_Dev_of_the_model" = round(c(
      ...
      boost_rf_sd_mean, cppls_sd_mean, cubist_sd_mean, earth_sd_mean, elastic_sd_mean, gam_sd_mean, gb_sd_mean, knn_sd_mean, lasso_sd_mean,
      linear_sd_mean, pls_sd_mean, pcr_sd_mean, rf_sd_mean, ridge_sd_mean,
    ...
    "Mean_train_RMSE" = round(c(
      ...
      boost_rf_train_RMSE_mean, cppls_train_RMSE_mean, cubist_train_RMSE_mean, earth_train_RMSE_mean, elastic_train_RMSE_mean, gam_train_RMSE_mean, gb_train_RMSE_mean,
      knn_train_RMSE_mean, lasso_train_RMSE_mean, linear_train_RMSE_mean,
    ...
    "Mean_test_RMSE" = round(c(
      ...
      bayesrnn_test_RMSE_mean, boost_rf_test_RMSE_mean, cppls_test_RMSE_mean, cubist_test_RMSE_mean, earth_test_RMSE_mean, elastic_test_RMSE_mean,
      gam_test_RMSE_mean, gb_test_RMSE_mean, knn_test_RMSE_mean, lasso_test_RMSE_mean, linear_test_RMSE_mean,
      pls_test_RMSE_mean, pcr_test_RMSE_mean, rf_test_RMSE_mean, ridge_test_RMSE_mean,
    ...
    "Mean_validation_RMSE" = round(c(
      ...
      bayesrnn_validation_RMSE_mean, boost_rf_validation_RMSE_mean, cppls_validation_RMSE_mean, cubist_validation_RMSE_mean, earth_validation_RMSE_mean, elastic_validation_RMSE_mean,
      gam_validation_RMSE_mean, gb_validation_RMSE_mean, knn_validation_RMSE_mean, lasso_validation_RMSE_mean,
      linear_validation_RMSE_mean, pls_validation_RMSE_mean,
    ...
    "Overfitting_mean" = round(c(
      ...
      boost_rf_overfitting_mean, cppls_overfitting_mean, cubist_overfitting_mean, earth_overfitting_mean, elastic_overfitting_mean, gam_overfitting_mean, gb_overfitting_mean,
      knn_overfitting_mean, lasso_overfitting_mean, linear_overfitting_mean,
    ...
    "Overfitting_sd" = round(c(
      ...
      boost_rf_overfitting_sd, cppls_overfitting_sd, cubist_overfitting_sd, earth_overfitting_sd, elastic_overfitting_sd, gam_overfitting_sd, gb_overfitting_sd,
      knn_overfitting_sd, lasso_overfitting_sd, linear_overfitting_sd,
    ...
    "Duration" = round(c(
      ...
      boost_rf_duration_mean, cppls_duration_mean, cubist_duration_mean, earth_duration_mean, elastic_duration_mean, gam_duration_mean, gb_duration_mean,
      knn_duration_mean, lasso_duration_mean, linear_duration_mean,
    ...
    "Duration_sd" = round(c(
      ...
      boost_rf_duration_sd, cppls_duration_sd, cubist_duration_sd, earth_duration_sd, elastic_duration_sd, gam_duration_sd, gb_duration_sd,
      knn_duration_sd, lasso_duration_sd, linear_duration_sd,
  ...
  overfitting_data <-
    data.frame(
      ...
      "model" = c(
        ...
        c(rep("BayesRNN", numresamples)), c(rep("Boost Random Forest", numresamples)), c(rep("CPPLS", numresamples)),
        ...
        c(rep("Principal Components Regression", numresamples)), c(rep("Partial Least Squares", numresamples)),
      ...
      "data" = c(
        ...
        bayesrnn_overfitting, boost_rf_overfitting, cppls_overfitting,
        ...
        pcr_overfitting, pls_overfitting,
      ...
      "mean" = rep(c(
        ...
        bayesrnn_overfitting_mean, boost_rf_overfitting_mean, cppls_overfitting_mean,
        ...
        pcr_overfitting_mean, pls_overfitting_mean,
  ...
  bias_data <-
    data.frame(
      ...
      "data" = c(
        ...
        bayesrnn_bias_mean, boost_rf_bias_mean, cppls_bias_mean,
  ...
  #### CPPLS visualizations
  cppls_df <- data.frame(
    actual = c(test$y, validation$y), predicted = as.numeric(y_hat_cppls),
    residuals = c(test$y, validation$y) - y_hat_cppls
  )

  cppls_pred_vs_actual <- ggplot2::ggplot(cppls_df, mapping = aes(x = actual, y = as.numeric(y_hat_cppls))) +
    ggplot2::geom_point() +
    ggplot2::geom_abline(intercept = 0, slope = 1, color = "red") +
    ggplot2::labs(title = "cppls model: Predicted vs actual", x = "Actual", y = "Predicted")

  cppls_resid_vs_actual <- ggplot2::ggplot(cppls_df, mapping = aes(x = actual, y = residuals)) +
    ggplot2::geom_point() +
    ggplot2::geom_hline(yintercept = 0, color = "red") +
    ggplot2::labs(title = "cppls model: Residuals", x = "Actual", y = "Predicted")

  cppls_hist_residuals <- ggplot2::ggplot(cppls_df, mapping = aes(x = residuals)) +
    ggplot2::geom_histogram(bins = round(nrow(df) / 10)) +
    ggplot2::geom_vline(xintercept = 0, color = "red") +
    ggplot2::labs(title = "cppls model: Histogram of residuals, each bar = 10 rows of data")

  cppls_qq <- ggplot2::ggplot(cppls_df, aes(sample = as.numeric(y_hat_cppls))) + ggplot2::stat_qq() +
    ggplot2::labs(title = "cppls model: Q-Q plot") +
    ggplot2::stat_qq_line(color = "red")
  ...
  k_s_test_barchart <- ggplot2::ggplot(summary_results, aes(x = reorder(Model, KS_Test_P_Value_mean, decreasing = TRUE), y = KS_Test_P_Value_mean)) +
    ...
    ggplot2::ylim(-0.25, 1.5*max(summary_results$KS_Test_P_Value_mean)) +
    ggplot2::geom_hline(yintercept = c(0.05, 0.10), linetype='dashed', color=c('blue', 'blue')) +
    ggplot2::annotate("text", x = 40, y = 0.07, label = "p=0.05") +
    ggplot2::annotate("text", x = 40, y = 0.12, label = "p=0.10") +
  ...
  if (data_visualizations[1] == "CPPLS") {
    grid.arrange(cppls_pred_vs_actual, cppls_resid_vs_actual, cppls_hist_residuals, cppls_qq, ncol = 2)
    gridExtra::grid.arrange(cppls_pred_vs_actual)
    gridExtra::grid.arrange(cppls_resid_vs_actual)
    gridExtra::grid.arrange(cppls_hist_residuals)
    gridExtra::grid.arrange(cppls_qq)
  }
  ...
  accuracy_data <-
    data.frame(
      ...
      "data" = c(
        ...
        bayesrnn_holdout_RMSE, boost_rf_holdout_RMSE, cppls_holdout_RMSE,
        ...
        pcr_holdout_RMSE, pls_holdout_RMSE,
      ...
      "mean" = rep(c(
        ...
        bayesrnn_holdout_RMSE_mean, boost_rf_test_RMSE_mean, cppls_holdout_RMSE_mean,
        ...
        pcr_holdout_RMSE_mean, pls_holdout_RMSE_mean,
  ...
  total_data <-
    data.frame(
      ...
      "train" = c(
        ...
        bayesrnn_train_RMSE, boost_rf_train_RMSE, cppls_train_RMSE,
  ...
  if (do_you_have_new_data == "Y") {
    ...
    new_cppls <- predict(object = cppls_train_fit, newdata = new_data)
    ...
    new_ensemble <- data.frame(
      ...
      "CPPLS" = new_cppls / cppls_holdout_mean,
    ...
    if (save_all_trained_models == "Y") {
      ...
      cppls_train_fit <<- cppls_train_fit # CPPLS models
