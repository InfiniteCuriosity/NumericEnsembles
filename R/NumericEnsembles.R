...
#' @importFrom stats na.omit var cor as.formula predict glm median sd rnorm runif rpois rgamma rbinom ks.test shapiro.test cor.test density lowess qqnorm qqline na.exclude lm reorder cooks.distance
...
if (getRversion() >= "2.15.1") {
  utils::globalVariables(c(
    ".data", "Actual", "Bias", "Correlation", "Feature", "In Sample RMSE",
    "Missing_Count", "Missing_Pct", "Model", "Out of Sample RMSE", "Overfitting_Ratio",
    "RMSE 95% CI Lower", "RMSE 95% CI Upper", "Residuals", "Type", "Unique_Values",
    "Value", "Var1", "Var2", "Variance", "Z_Score", "name", "value",
    "Testing_RMSE", "Testing_MAE", "Adjusted_R2", "Overfitting", "KS_p_value",
    "Metric", "Absolute_Bias", "MAE 95% CI Lower", "MAE 95% CI Upper",
    "Adjusted R2 95% CI Lower", "Adjusted R2 95% CI Upper", "Feature_Name",
    "Data_Type", "Missing_Rate", "Skewness_Coef", "Outliers_Found", "Operational_Insight",
    "Index", "Distance", "Outlier"
  ))
}
...
#' @param train_pct Numeric. The training data proportion as a decimal between 0 and 1. Default is 0.60.
#' @param vif_threshold Numeric. Variance Inflation Factor threshold for filtering multicollinear predictors. Default is 5.
#' @param transform_steps Character vector. Preprocessing transforms to pass to caret. Default is c("nzv", "medianImpute", "center", "scale", "YeoJohnson").
#' @param rf_grid Optional custom tuning grid for Random Forest models. Default is NULL.
#' @param glmnet_grid Custom tuning grid for Elastic Net models.
#' @param svm_tune_length Integer. Tuning length parameter for SVM models. Default is 8.
#' @param mars_tune_length Integer. Tuning length parameter for MARS models. Default is 5.
#' @param pcr_tune_length Integer. Tuning length parameter for PCR/PLS models. Default is 10.
#' @param tree_tune_length Integer. Tuning length parameter for tree-based models. Default is 10.
#' @param cooks_threshold Numeric. Leverage cutoff multiplier relative to 4/n benchmark threshold. Values between 1-5 enforce pruning; 999 deactivates filter. Default is 999.
#'
...
#' Fast-Execution Configuration Matrix for Regression Testing
...
# -------------------------------------------------------------------------
# 2. INTERNAL MODULE GRAPHICS & ANALYTICAL SUB-ENGINES
# -------------------------------------------------------------------------
...
#' Comprehensive Continuous Regression Ensemble Pipeline Engine
#'
#' Orchestrates high-velocity multi-model exploration across continuous regression domains,
#' managing automated data dictionary profiling, iterative collinearity pruning, model training,
#' competitive stacking meta-learners, and advanced visual diagnostics matrices.
#'
#' @param dataset A data frame containing the predictors and target variable.
#' @param target_col Character string specifying the name of the target continuous numeric column.
#' @param facet_col Character string specifying an optional categorical column to facet EDA charts by. Default is "".
#' @param color_col Character string specifying an optional column to color exploratory EDA charts by. Default is "".
#' @param stratify_col Character string specifying an optional categorical column to anchor stratified sampling splits. Default is "".
#' @param palette_style Character choice layout. Visual color scheme for plot vectors: "standard", "viridis", or "modern".
#' @param config A pre-configured architecture configuration parameter matrix list from \code{NumericEnsemblesConfig()}.
#' @param verbose Logical. If TRUE, logs operational pipeline milestones to console. Default is TRUE.
#'
#' @return An object of class \code{numeric_pipeline} containing complete metrics matrices, model instances, data dictionaries, and analytical plots.
#' @export
Numeric <- function(dataset = NULL,
  ...
  required_packages <- c("caret", "MASS", "rpart", "Cubist", "nnet", "ipred",
                         "elasticnet", "glmnet", "randomForest", "kernlab", "brnn",
                         "earth", "pls", "quantregForest", "party", "car", "partykit",
                         "doParallel", "ggplot2", "patchwork", "tidyr", "ggrepel",
                         "tidyselect", "gam")
  ...
  if (is.null(dataset)) {
    if (interactive()) {
      ...
      df <- tryCatch({ eval(parse(text = dataset_name)) }, error = function(e) { stop("Could not find or load the specified dataset.", call. = FALSE) })
    } else {
      stop("Argument 'dataset' matrix must be supplied explicitly in non-interactive execution modes.", call. = FALSE)
    }
  } else {
  ...
  if (is.null(target_col)) {
    if (interactive()) {
      ...
      stop("Define the primary numeric target column explicitly in non-interactive execution modes.", call. = FALSE)
    }
  }
  if (!(target_col %in% colnames(df))) stop("Target continuous numeric column not found inside dataset assets.", call. = FALSE)
  if (!is.numeric(df[[target_col]])) stop("Target column must be numeric for continuous regression ensembles.", call. = FALSE)

  if (stratify_col != "") {
    if (!(stratify_col %in% colnames(df))) stop(sprintf("Stratification column '%s' not found inside dataset assets.", stratify_col), call. = FALSE)
    strat_val <- df[[stratify_col]]
    if (is.numeric(strat_val) && length(unique(stats::na.omit(strat_val))) > 15) {
      stop(sprintf("Column '%s' is continuous numeric. Stratified split structures require discrete factor properties.", stratify_col), call. = FALSE)
    }
  }

  palette_style <- match.arg(palette_style)
  theme_colors <- switch(palette_style,
                         "standard" = list(primary = "steelblue", secondary = "cyan4", accent = "purple", highlight = "darkgreen", warning = "tomato", ks_fill = "darkorange2", ks_line1 = "blue", ks_line2 = "red", tiles_low = "darkgreen", tiles_mid = "white", tiles_high = "darkred"),
                         "viridis"  = list(primary = "#21918c", secondary = "#3b528b", accent = "#440154", highlight = "#5dc963", warning = "#fde725", ks_fill = "#21918c", ks_line1 = "#440154", ks_line2 = "#fde725", tiles_low = "#440154", tiles_mid = "#21918c", tiles_high = "#fde725"),
                         "modern"   = list(primary = "#111E6C", secondary = "#FF6F61", accent = "#008080", highlight = "#708090", warning = "#FF4500", ks_fill = "#FF6F61", ks_line1 = "#111E6C", ks_line2 = "#FF4500", tiles_low = "#008080", tiles_mid = "#ECEFF1", tiles_high = "#FF6F61")
  )
  ...
  # --- Upgraded: Generating High-Resolution Insights Table Structure ---
  insights_rows <- list()
  for (col in colnames(df)) {
    if (is.numeric(df[[col]])) {
      vals <- stats::na.omit(df[[col]])
      q_3 <- summary(vals)[5]; q_1 <- summary(vals)[2]; iqr <- q_3 - q_1
      out_cnt <- sum(vals > (q_3 + 1.5 * iqr) | vals < (q_1 - 1.5 * iqr))

      mean_v <- mean(vals); sd_v <- stats::sd(vals)
      skew <- if(sd_v > 0) mean((vals - mean_v)^3) / (sd_v^3) else 0

      status_str <- "Structural Signature: Healthy"
      if (abs(skew) > 1.5) status_str <- "High Structural Tail Skewness Risk"
      if (out_cnt > (0.10 * length(vals))) status_str <- paste0(status_str, " / Severe Outlier Density")
      if (sd_v == 0) status_str = "Zero Variance Anomaly (Constant)"

      insights_rows[[col]] <- data.frame(
        Feature_Name        = col,
        Data_Type           = "Numeric Continuous",
        Missing_Rate        = paste0(round((sum(is.na(df[[col]])) / nrow(df)) * 100, 2), "%"),
        Skewness_Coef       = round(skew, 2),
        Outliers_Found      = as.integer(out_cnt),
      ...
      insights_rows[[col]] <- data.frame(
        Feature_Name        = col,
        Data_Type           = paste(class(df[[col]]), collapse = ", "),
        Missing_Rate        = paste0(round((sum(is.na(df[[col]])) / nrow(df)) * 100, 2), "%"),
        Skewness_Coef       = NA,
        Outliers_Found      = 0L,
        Operational_Insight = "Discrete Feature / Dummy Pipeline Required",
        stringsAsFactors    = FALSE
      )
    }
  }
  ...
  p_hist <- ggplot2::ggplot(df_long_all, ggplot2::aes(x = Value)) +
    ...
    ggplot2::facet_wrap(~Feature, scales = "free") + ggplot2::theme_minimal() +
  ...
  scatter_cols <- setdiff(colnames(df)[sapply(df, is.numeric)], target_col)
  ...
  p_scatter <- ggplot2::ggplot(df_scatter_long, ggplot2::aes(x = Value, y = .data[[target_col]])) +
    ...
    ggplot2::facet_wrap(~Feature, scales = "free_x") + ggplot2::theme_minimal() +
  ...
  # --- PSOCK Concurrency Grid Safeguards ---
  ...
  # --- Automated Outlier Cook's Distance Calculation & Plotting Matrix ---
          cook_fit <- stats::lm(stats::as.formula(paste(target_col, " ~ .")), data = train_data)
          cooks_d <- stats::cooks.distance(cook_fit)
          cutoff <- (if(config$cooks_threshold < 900) config$cooks_threshold else 4) * (4 / nrow(train_data))

          cooks_df <- data.frame(Index = seq_along(cooks_d), Distance = cooks_d, Outlier = cooks_d >= cutoff)

          p_cooks <- ggplot2::ggplot(cooks_df, ggplot2::aes(x = Index, y = Distance)) +
            ggplot2::geom_segment(ggplot2::aes(xend = Index, yend = 0, color = Outlier), alpha = 0.6) +
            ggplot2::geom_point(ggplot2::aes(color = Outlier), size = 1) +
            ggplot2::geom_hline(yintercept = cutoff, color = theme_colors$warning, linetype = "dashed", linewidth = 0.8) +
            ggplot2::scale_color_manual(values = c("FALSE" = theme_colors$primary, "TRUE" = theme_colors$warning)) +
            ggplot2::theme_minimal() +
            ggplot2::labs(title = "Cook's Distance Leverage Influencer Diagnosis", x = "Observation Index", y = "Cook's Distance") +
            ggplot2::theme(plot.title = ggplot2::element_text(face = "bold", size = 12), legend.position = "none")

          if (config$cooks_threshold < 900) {
            if (verbose) cat(sprintf("\n[Leverage Engine]: Pruning %d structural outliers via Cook's Distance cutoff (%.5f)...\n", sum(cooks_df$Outlier), cutoff))
            train_data <- train_data[!cooks_df$Outlier, ]
          }
            ...
            while(TRUE) {
              ...
              if (max_vif > config$vif_threshold) {
                ...
                if(is.matrix(vif_values)) {
                  for(f in rownames(vif_values)) vif_report_table <- rbind(vif_report_table, data.frame(Feature = f, VIF = round(vif_values[f, "GVIF"], 2), Status = "Kept", stringsAsFactors = FALSE))
                } else {
                  for(f in names(vif_values)) vif_report_table <- rbind(vif_report_table, data.frame(Feature = f, VIF = round(vif_values[f], 2), Status = "Kept", stringsAsFactors = FALSE))
                }
                break
              }
            }
          ...
          rf_grid_dispatch <- if (is.null(config$rf_grid)) expand.grid(mtry = intersect(c(2, 4, 6, 8), 1:final_predictor_count)) else config$rf_grid
          ...
          v_model <- character(n_models); v_testing_rmse <- numeric(n_models); v_rmse_ci_lower <- numeric(n_models); v_rmse_ci_upper <- numeric(n_models)
          v_testing_mae <- numeric(n_models); v_mae_ci_lower <- numeric(n_models); v_mae_ci_upper <- numeric(n_models); v_adj_r2 <- numeric(n_models)
          v_adj_r2_lower <- numeric(n_models); v_adj_r2_upper <- numeric(n_models); v_duration <- numeric(n_models); v_overfitting <- numeric(n_models)
          v_bias <- numeric(n_models); v_variance <- numeric(n_models); v_ks_p <- numeric(n_models)
          top_pred_names <- list()

          for (i in seq_len(n_models)) {
            model_name <- m_names[i]; mod <- models_list[[model_name]]
            pred_train <- pred_train_list[[model_name]]; pred_test <- pred_test_list[[model_name]]

            rmse_train <- sqrt(mean((actual_train - pred_train)^2)); rmse_test <- sqrt(mean((actual_test - pred_test)^2))
            mae_test   <- mean(abs(actual_test - pred_test))

            test_residuals <- actual_test - pred_test; se_mse <- stats::sd(test_residuals^2) / sqrt(length(test_residuals)); mse_test <- mean(test_residuals^2)
            rmse_ci_lower <- sqrt(max(0, mse_test - (1.96 * se_mse))); rmse_ci_upper <- sqrt(mse_test + (1.96 * se_mse))

            abs_errors <- abs(test_residuals); se_mae <- stats::sd(abs_errors) / sqrt(length(abs_errors))
            mae_ci_lower <- max(0, mae_test - (1.96 * se_mae)); mae_ci_upper <- mae_test + (1.96 * se_mae)

            ss_res_test <- sum(test_residuals^2); r2_val <- if (ss_tot_test > 0) 1 - (ss_res_test / ss_tot_test) else 0
            adj_r2_val <- if (n_test > p_features + 1) 1 - ((1 - r2_val) * (n_test - 1) / (n_test - p_features - 1)) else r2_val

            r2_ci_lower <- 1 - ((mse_test + (1.96 * se_mse)) / var_actual_test); r2_ci_upper <- 1 - ((max(0, mse_test - (1.96 * se_mse))) / var_actual_test)
            adj_r2_lower_val <- if (n_test > p_features + 1) 1 - ((1 - r2_ci_lower) * (n_test - 1) / (n_test - p_features - 1)) else r2_ci_lower
            adj_r2_upper_val <- if (n_test > p_features + 1) 1 - ((1 - r2_ci_upper) * (n_test - 1) / (n_test - p_features - 1)) else r2_ci_upper

            v_model[i] <- model_name; v_testing_rmse[i] <- round(rmse_test, 4); v_rmse_ci_lower[i] <- round(rmse_ci_lower, 4); v_rmse_ci_upper[i] <- round(rmse_ci_upper, 4)
            v_testing_mae[i] <- round(mae_test, 4); v_mae_ci_lower[i] <- round(mae_ci_lower, 4); v_mae_ci_upper[i] <- round(mae_ci_upper, 4)
            v_adj_r2[i] <- round(adj_r2_val, 4); v_adj_r2_lower[i] <- round(max(0, adj_r2_lower_val), 4); v_adj_r2_upper[i] <- round(min(1, adj_r2_upper_val), 4)
            v_duration[i] <- round(durations_list[[model_name]], 4); v_overfitting[i] <- round(rmse_test / rmse_train, 4)
            v_bias[i] <- round(mean(pred_test - actual_test), 4); v_variance[i] <- round(stats::var(pred_test), 4)

            ks_res <- suppressWarnings(stats::ks.test(pred_test, actual_train))
            v_ks_p[i] <- round(ks_res$p.value, 4)

            top_pred_names[[model_name]] <- tryCatch({
              imp <- caret::varImp(mod); imp_df = if (!is.null(imp$importance)) imp$importance else imp
              rownames(imp_df)[order(rowSums(as.matrix(imp_df)), decreasing = TRUE)[1]]
            }, error = function(e) { colnames(train_data)[colnames(train_data) != target_col][1] })
          }

          report = data.frame(
            Model = v_model, Testing_RMSE = v_testing_rmse, `RMSE 95% CI Lower` = v_rmse_ci_lower, `RMSE 95% CI Upper` = v_rmse_ci_upper,
          ...
          # --- Stacking Meta-Learner Layer Engine ---
          ...
          start_t <- proc.time(); meta_grid <- expand.grid(alpha = seq(0, 1, length = 11), lambda = 10^seq(-4, -1, length = 30))
          ...
          pred_train_list[["Meta_GLM"]]  <- as.numeric(stats::predict(meta_model_glm, newdata = meta_train_df)); pred_test_list[["Meta_GLM"]]   <- as.numeric(stats::predict(meta_model_glm, newdata = meta_test_df))
          pred_train_list[["Meta_Enet"]] <- as.numeric(stats::predict(meta_model_enet, newdata = meta_train_df)); pred_test_list[["Meta_Enet"]]  <- as.numeric(stats::predict(meta_model_enet, newdata = meta_test_df))
          pred_train_list[["Meta_GAM"]]  <- as.numeric(stats::predict(meta_model_gam, newdata = meta_train_df)); pred_test_list[["Meta_GAM"]]   <- as.numeric(stats::predict(meta_model_gam, newdata = meta_test_df))
          pred_train_list[["Meta_PLS"]]  <- as.numeric(stats::predict(meta_model_pls, newdata = meta_train_df)); pred_test_list[["Meta_PLS"]]   <- as.numeric(stats::predict(meta_model_pls, newdata = meta_test_df))
          pred_train_list[["Meta_RF"]]   <- as.numeric(stats::predict(meta_model_rf, newdata = meta_train_df)); pred_test_list[["Meta_RF"]]    <- as.numeric(stats::predict(meta_model_rf, newdata = meta_test_df))
          pred_train_list[["Meta_SVM"]]  <- as.numeric(stats::predict(meta_model_svm, newdata = meta_train_df)); pred_test_list[["Meta_SVM"]]   <- as.numeric(stats::predict(meta_model_svm, newdata = meta_test_df))

          models_list[["Meta_GLM"]] <- meta_model_glm; models_list[["Meta_Enet"]] <- meta_model_enet; models_list[["Meta_GAM"]] <- meta_model_gam
          models_list[["Meta_PLS"]] <- meta_model_pls; models_list[["Meta_RF"]]   <- meta_model_rf;   models_list[["Meta_SVM"]] <- meta_model_svm
          ...
          for (m_name in meta_names) {
            ...
            ks_m <- suppressWarnings(stats::ks.test(p_test, actual_train))
            ...
            meta_reports[[m_name]] <- data.frame(
              ...
              Adjusted_R2 = round(adj_r2_m, 4), `Adjusted R2 95% CI Lower` = round(max(0, adj_r2_low_m), 4), `Adjusted R2 95% CI Upper` = round(min(1, adj_r2_upp_m), 4),
              Duration = round(meta_durations[[m_name]], 4), Overfitting = round(rmse_test / rmse_train, 4), Bias = round(mean(p_test - actual_test), 4),
              Variance = round(stats::var(p_test), 4), KS_p_value = round(ks_m$p.value, 4), stringsAsFactors = FALSE, check.names = FALSE
            )
          }
          master_reporting_base <- rbind(report, do.call(rbind, meta_reports))

          # --- Blended Pairwise Combination Optimization Engine ---
          pairs_grid <- utils::combn(m_names, 2, simplify = FALSE); n_pairs <- length(pairs_grid); ens_reports <- list()
          if (verbose) pb <- utils::txtProgressBar(min = 0, max = n_pairs, style = 3)
          ...
          for (idx in seq_along(pairs_grid)) {
            ...
            ks_c <- suppressWarnings(stats::ks.test(p_test_comb, actual_train))
            ...
            ens_reports[[idx]] <- data.frame(
              ...
              Adjusted_R2 = round(adj_r2_c, 4), `Adjusted R2 95% CI Lower` = round(max(0, adj_r2_low_c), 4), `Adjusted R2 95% CI Upper` = round(min(1, adj_r2_upp_m), 4),
              ...
              Variance = round(stats::var(p_test_comb), 4), KS_p_value = round(ks_c$p.value, 4), stringsAsFactors = FALSE, check.names = FALSE
            )
            if (verbose) utils::setTxtProgressBar(pb, idx)
          }
          if (verbose) close(pb)

          ens_master_df <- do.call(rbind, ens_reports); ens_master_df <- ens_master_df[order(ens_master_df$Testing_RMSE), ]
          report <- rbind(master_reporting_base, utils::head(ens_master_df, 10))
          report <- report[order(report$Testing_RMSE), ]; rownames(report) <- NULL

          # --- Automated Residual Diagnostic Audit Matrix ---
          audit_logs <- list()
          for (m_name in report$Model) {
            p_test <- pred_test_list[[m_name]]; res_test <- actual_test - p_test
            ...
            audit_logs[[m_name]] <- data.frame(Model = m_name, Residual_Normality = norm_status, Variance_Stability = het_status, Error_Independence = ac_status, stringsAsFactors = FALSE)
          }
          audit_report_matrix <- do.call(rbind, audit_logs); rownames(audit_report_matrix) <- NULL

          # --- Visualizations Matrix Generation ---
          p_kpi_metrics <- (.make_metric_plot(report, "Testing_RMSE", "Testing RMSE (Lower is Better)", theme_colors$primary, theme_colors, show_ci = TRUE, ci_lower_col = "RMSE 95% CI Lower", ci_upper_col = "RMSE 95% CI Upper") +
            ...
            patchwork::plot_layout(ncol = 3) + patchwork::plot_annotation(title = "Core Model Performance Metrics & KPIs", theme = ggplot2::theme(plot.title = ggplot2::element_text(size = 14, face = "bold", hjust = 0.5)))
          ...
          p_risk_metrics <- (.make_metric_plot(report, "Overfitting", "Overfitting Ratio (Ideal = 1.0)", theme_colors$warning, theme_colors, is_overfit = TRUE) +
            ...
            patchwork::plot_layout(ncol = 2) + patchwork::plot_annotation(title = "Generalization Risks and Structural Bias Diagnostics", theme = ggplot2::theme(plot.title = ggplot2::element_text(size = 14, face = "bold", hjust = 0.5)))
          ...
          output_results <- list(
            performance_report = report, audit_report = audit_report_matrix, base_models = models_list,
            meta_models = list(GLM = models_list[["Meta_GLM"]], Enet = models_list[["Meta_Enet"]], GAM = models_list[["Meta_GAM"]], PLS = models_list[["Meta_PLS"]], RF = models_list[["Meta_RF"]], SVM = models_list[["Meta_SVM"]]),
            ...
            pipeline_meta = list(
              target_col = target_col, stratify_col = stratify_col, kept_features = kept_vif_features, dummy_model = dummy_model, palette_style = palette_style, train_data_ref = train_data,
              top_3_models = top_3_models, pred_test_list = pred_test_list, pred_train_list = pred_train_list, actual_test = actual_test, actual_train = actual_train, test_data = test_data,
              theme_colors = theme_colors, top_pred_names = top_pred_names
            ),
            plots = list(
              histograms = eda_plots$histograms, boxplots = eda_plots$boxplots, correlation = eda_plots$correlation, scatter_matrix = eda_plots$scatter_matrix,
              metric_heatmap = eda_plots$metric_heatmap, kpis = p_kpi_metrics, risks = p_risk_metrics, tradeoff = p_tradeoff_assembled, ks_test = p_ks_assembled, cooks_distance = p_cooks,
  ...
  #' Print Numeric Pipeline Summary Profiles Report
  #'
  ...
  print.numeric_pipeline <- function(x, ...) {
    cat("\n=========================================================================\n")
    cat("                  NUMERIC PIPELINE PIPELINE PROFILE EXPORTS             \n")
    cat("=========================================================================\n")
    ...
    if (is.data.frame(x$vif_report)) { print(x$vif_report, row.names = FALSE) } else { cat("  ->", x$vif_report, "\n") }
    ...
    print(utils::head(x$performance_report[, c("Model", "Testing_RMSE", "Testing_MAE", "Adjusted_R2", "Variance", "KS_p_value", "Overfitting", "Bias", "Duration")], 10), row.names = FALSE)
    ...
    if (any(utils::head(x$audit_report$Variance_Stability, 3) == "Heteroscedastic (Unstable Variance)")) {
      cat("\n[Audit Alert]: Heteroscedasticity caught in leader zone. Upper intervals could degrade.\n")
    }
    if (any(utils::head(x$audit_report$Residual_Normality, 3) == "Non-Normal (Biased Tail Risks)")) {
      cat("[Audit Alert]: Non-normal residuals mapped in leader zone. Points possess fat tails.\n")
    }
  }
  ...
  #'
  #' @param x A numeric_pipeline object generated by the Numeric() engine.
  #' @param pace_output Logical. If TRUE and session is interactive, paces chart rendering frames via devAskNewPage. Default is TRUE.
  ...
  plot.numeric_pipeline <- function(x, pace_output = TRUE, ...) {
    ...
    if (!is.null(x$plots$cooks_distance)) print(x$plots$cooks_distance)
  ...
  #'
  #' @param object A numeric_pipeline object trained by the engine.
  #' @param newdata A data frame containing incoming new samples to score.
  ...
  predict.numeric_pipeline <- function(object, newdata, model_name = "best", ...) {
    ...
    if (!is.null(object$pipeline_meta$dummy_model)) {
      ...
      if (is.null(expected_variables) && "lvls" %in% names(dm)) expected_variables <- names(dm$lvls)
    }
    if (is.null(expected_variables)) {
      expected_variables <- setdiff(colnames(object$pipeline_meta$train_data_ref), object$pipeline_meta$target_col)
    }
    ...
    if (grepl("Meta_", model_name)) {
      ...
      base_preds <- lapply(object$base_models[1:17], function(mod) as.numeric(stats::predict(mod, newdata = new_encoded, na.action = stats::na.pass)))
      return(as.numeric(stats::predict(object$meta_models[[meta_key]], newdata = as.data.frame(base_preds))))
    }
  ...
  #'
  #' @title Executive Production Projections with Row-Level 95 percent Confidence Intervals
  #' @description Evaluates champion architectures against un-indexed datasets, appending row-level projection metrics and interval boundaries.
  #'
  #' @param object A trained \code{numeric_pipeline} asset.
  #' @param newdata A data frame containing incoming raw manufacturing features.
  #'
  #' @return A data frame matching input dimensions appended with prediction vectors and confidence interval boundaries for the top 3 models.
  #' @export
  predict_production <- function(object, newdata) {
    ...
    for (i in seq_along(top_3_names)) {
      ...
      production_report[[sprintf("Rank_%d_%s_95_LowerBound", i, clean_m_label)]] <- round(point_predictions - (1.96 * sigma_model), 2)
      production_report[[sprintf("Rank_%d_%s_95_UpperBound", i, clean_m_label)]] <- round(point_predictions + (1.96 * sigma_model), 2)
    }
    return(production_report)
  }

  #' Compress and Save Trained Pipeline Assets
  #'
  #' @param object A trained \code{numeric_pipeline} asset.
  ...
  save_pipeline <- function(object, file_path) {
    ...
    saved_bundle <- list(performance_report = object$performance_report, audit_report = object$audit_report, base_models = object$base_models, meta_models = object$meta_models, pipeline_meta = object$pipeline_meta, exploratory_insights = object$exploratory_insights)
  ...
  #'
  #' @return A deserialized \code{numeric_pipeline} object.
  #' @export
  load_pipeline <- function(file_path) {
    ...
    if (!inherits(bundle, "serialized_numeric_pipeline")) stop("File does not contain a valid footprint.", call. = FALSE)
    class(bundle) <- "numeric_pipeline"
    return(bundle)
  }

  #' Export Tables and High-Resolution Visualization Panels
  #'
  #' @param pipeline_object A trained \code{numeric_pipeline} asset.
  #' @param export_directory Character path detailing target output folder destination.
  #'
  #' @return Invisible string path location where files were compiled.
  #' @export
  ExportNumericResults <- function(pipeline_object = NULL, export_directory = NULL) {
    if (is.null(pipeline_object)) stop("Must provide a valid pipeline return asset.", call. = FALSE)
    save_dir <- if (is.null(export_directory)) tempdir() else export_directory
    if (!dir.exists(save_dir)) dir.create(save_dir, recursive = TRUE)

    utils::write.csv(pipeline_object$performance_report, file = file.path(save_dir, "numeric_performance_report.csv"), row.names = FALSE)
    utils::write.csv(pipeline_object$data_dictionary, file = file.path(save_dir, "data_dictionary_schema.csv"), row.names = FALSE)
    utils::write.csv(pipeline_object$vif_report, file = file.path(save_dir, "vif_collinearity_pruning_report.csv"), row.names = FALSE)
    utils::write.csv(pipeline_object$exploratory_insights, file = file.path(save_dir, "exploratory_data_insights.csv"), row.names = FALSE)

    if (!is.null(pipeline_object$plots$histograms)) ggplot2::ggsave(filename = file.path(save_dir, "01_feature_histograms.png"), plot = pipeline_object$plots$histograms, width = 11, height = 8, dpi = 300)
    if (!is.null(pipeline_object$plots$correlation)) ggplot2::ggsave(filename = file.path(save_dir, "02_linear_correlation_heatmap.png"), plot = pipeline_object$plots$correlation, width = 9, height = 7, dpi = 300)
    if (!is.null(pipeline_object$plots$kpis)) ggplot2::ggsave(filename = file.path(save_dir, "03_leaderboard_kpis.png"), plot = pipeline_object$plots$kpis, width = 14, height = 7, dpi = 300)
    if (!is.null(pipeline_object$plots$risks)) ggplot2::ggsave(filename = file.path(save_dir, "04_generalization_risks.png"), plot = pipeline_object$plots$risks, width = 11, height = 7, dpi = 300)
    if (!is.null(pipeline_object$plots$tradeoff)) ggplot2::ggsave(filename = file.path(save_dir, "05_bias_variance_tradeoff.png"), plot = pipeline_object$plots$tradeoff, width = 11, height = 7, dpi = 300)
    if (!is.null(pipeline_object$plots$ks_test)) ggplot2::ggsave(filename = file.path(save_dir, "06_ks_distribution_test.png"), plot = pipeline_object$plots$ks_test, width = 11, height = 7, dpi = 300)
    if (!is.null(pipeline_object$plots$cooks_distance)) ggplot2::ggsave(filename = file.path(save_dir, "07_cooks_distance_outliers.png"), plot = pipeline_object$plots$cooks_distance, width = 11, height = 6, dpi = 300)

    return(invisible(save_dir))
  }

  #' Compile and Render Automated Executive Summary Report
  #'
  #' @param pipeline_object A trained \code{numeric_pipeline} asset.
  #' @param output_directory Character path mapping where summary files are rendered.
  #'
  #' @return Invisible NULL.
  #' @export
  RenderExecutiveReport <- function(pipeline_object, output_directory = getwd()) {
    if (is.null(pipeline_object) || !inherits(pipeline_object, "numeric_pipeline")) stop("Must provide a valid pipeline return asset.", call. = FALSE)
    if (!requireNamespace("quarto", quietly = TRUE)) stop("The 'quarto' package is required to compile this document.", call. = FALSE)

    qmd_path <- file.path(output_directory, "Executive_Numeric_Regression_Report.qmd")
    rds_path <- file.path(output_directory, "numeric_pipeline_data_cache.rds")
    saveRDS(pipeline_object, file = rds_path)

    qmd_content <- c(
      "---", "title: 'Enterprise Continuous Regression & Stacking Ensemble Report'", "author: 'Russ Conte'", "date: '`r Sys.Date()`'",
      "format:", "  html:", "    theme: cosmo", "    toc: true", "    toc-depth: 2", "---", "",
      "```{r setup, include=FALSE}", sprintf("pipeline <- readRDS('%s')", gsub("\\\\", "/", rds_path)), "```", "",
      "## 1. Executive Summary", "The competitive parallelized regression ensemble pipeline successfully cross-validated models across the continuous space.",
      ...
      "## 2. Predictor Variable Properties & Insights", "```{r show-dict, echo=FALSE}", "knitr::kable(pipeline$exploratory_insights, caption = 'Automated Pipeline Exploratory Insights Matrix')", "```", "",
      "## 3. Algorithm Standings Leaderboard", "```{r show-leaderboard, echo=FALSE}", "knitr::kable(head(pipeline$performance_report, 15), caption = 'Team of Rivals Regression Standings Leaderboard')", "```"
    )
    writeLines(qmd_content, qmd_path); quarto::quarto_render(input = qmd_path, quiet = TRUE)
    if (file.exists(rds_path)) file.remove(rds_path)
  }

  #' Run Fast Validation Regression Demo Suite
  #'
  #' @return An invisible trained \code{numeric_pipeline} object run over simulated macroeconomic metrics.
  #' @export
  NumericEnsemblesDemo <- function() {
    message("Initializing NumericEnsembles Comprehensive Validation Demo...")
    set.seed(42); n_samples <- 250
    new_housing_index <- rnorm(n_samples, mean = 210, sd = 35); new_unemploy_rate <- runif(n_samples, min = 0.03, max = 0.12)
    new_gdp_growth <- 2.5 + (0.04 * new_housing_index) - (0.35 * new_unemploy_rate) + rnorm(n_samples, sd = 0.5)

    economic_registry <- data.frame(GDP_Growth = new_gdp_growth, Housing_Index = new_housing_index, Unemployment = new_unemploy_rate)
    test_run <- Numeric(dataset = economic_registry, target_col = "GDP_Growth", palette_style = "modern", config = NumericEnsemblesFastConfig(), verbose = TRUE)
    print(test_run)
    return(invisible(test_run))
  }

  #' Launch Interactive Shiny Application Portal
  #'
  #' @return Activated browser session running the dashboard interface.
  #' @export
  LaunchNumericApp <- function() {
    app_path <- base::system.file("shiny-apps", "NumericEnsemblesApp", package = "NumericEnsembles")
    if (app_path == "") stop("Shiny application dashboard directory not found within package files library installation.", call. = FALSE)
    if (!requireNamespace("shiny", quietly = TRUE)) stop("Package 'shiny' must be installed to activate the web interface app framework.", call. = FALSE)

    launch_opt <- getOption("shiny.launch.browser", TRUE); is_launch_active = if (is.logical(launch_opt)) launch_opt else TRUE
